...
Fase 1 — de onde vêm os achados. A skill busca em duas fontes, que podem coexistir: o arquivo inspecao-fisica.md (o relato de campo produzido pela /aft-inspecao-fisica) e a seção "## Anotações da auditoria" do memory.md (as constatações que você foi lançando durante a análise dos documentos: SESMT subdimensionado, ASO faltando, PGR inexistente). Se não houver nenhuma das duas, ela pede que você cole os achados.
