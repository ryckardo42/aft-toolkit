...
No memory.md ficam os dados duráveis da OS: números da OS e da demanda, vencimento, endereço, telefone, CNAE, grau de risco, efetivo e o quadro de pessoal (homens, mulheres, PCD, aprendizes, menores de 18), o registro de atividade e as pendências. As ementas ficam num arquivo separado, o ementas.md, com código e descrição literais — e ele é mais que uma lista: é a folha de resposta do item 2.5 do Relatório de Inspeção, onde cada ementa recebe a situação encontrada (regular, irregular, não aplicável, não fiscalizada) e, sendo irregular, as ações aplicadas (autuação, notificação, interdição, embargo, regularizada...). Como o memory.md é lido no começo de toda conversa da empresa e a lista de ementas só interessa em três momentos (preparação, enquadramento e encerramento), separar os dois deixa a conversa mais leve e a folha mais útil
