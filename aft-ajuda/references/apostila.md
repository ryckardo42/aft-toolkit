...
No memory.md ficam os dados duráveis da OS: números da OS e da demanda, vencimento, endereço, telefone, CNAE, grau de risco, efetivo e o quadro de pessoal (homens, mulheres, PCD, aprendizes, menores de 18), o registro de atividade e as pendências. As ementas ficam num arquivo separado, o ementas.md, com código e descrição literais — e ele é mais que uma lista: é o espelho do item 2.5 do Relatório de Inspeção, na mesma ordem da tela do SFIT (um grupo por Atributo/NR, em ordem alfabética, com asterisco nas ementas da OS), onde cada ementa recebe a situação encontrada (regular, irregular, não aplicável, não fiscalizada) e, sendo irregular, as ações aplicadas (autuação, notificação, interdição, embargo, regularizada...). No painel, o dossiê da auditoria traz um botão "abrir o espelho do SFIT" logo abaixo do endereço: ele abre a tabela em endereço próprio, para você deixar numa janela ao lado do SFIT-WEB enquanto digita. Como o memory.md é lido no começo de toda conversa da empresa e a lista de ementas só interessa em três momentos (preparação, enquadramento e encerramento), separar os dois deixa a conversa mais leve e a folha mais útil
...
Fase 1 — de onde vêm os achados. A skill busca em duas fontes, que podem coexistir: o arquivo inspecao-fisica.md (o relato de campo produzido pela /aft-inspecao-fisica) e a seção "## Auditoria de documentos" do memory.md (as constatações que você foi lançando durante a análise dos documentos: SESMT subdimensionado, ASO faltando, PGR inexistente; nas OS mais antigas a seção se chama "## Anotações da auditoria", e a skill lê as duas). Nas subseções temáticas da mesma seção (### PGR, ### Jornada...), o que há é um resumo com o ponteiro do relatório completo na pasta auditoria-<tema> — a skill segue o ponteiro e lê o relatório, sem tratar o resumo como constatação. Se não houver nenhuma das duas fontes, ela pede que você cole os achados.
