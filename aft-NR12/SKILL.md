...
6. **Feche o bloco II com o parágrafo de dano coletivo** (texto canônico abaixo) — toda infração de NR-12 é SST:
   ...
   A frase final ("Contudo, cita-se...") é a regra, não o enfeite: se a narrativa identificar o operador da máquina ou outro trabalhador exposto, cite esse(s), com a função se conhecida; havendo mais de um exposto identificado, use o plural ("Contudo, citam-se como exemplos de trabalhadores prejudicados [[TRAB_01]], [função], e [[TRAB_02]], [função]."). A relação de vínculos NÃO escolhe o exemplo: ela diz quem é empregado, não quem estava exposto, e "função compatível com a exposição" é presunção, não fato. Constar de relação de vínculos, de certificado de capacitação, de ficha de EPI ou de lista de presença não é registro de exposição. Sem esse registro, encerre em "...(Orientação Técnica SIT nº 2/2022).", sem a frase final, e declare a falta ao AFT — a constatação é dele. Nome em capitalização normal, podendo abreviar (primeiro nome + um sobrenome); função em minúsculas; **nunca cite CPF neste parágrafo**. Vale a anonimização por tokens `[[TRAB_NN]]` (registre a forma abreviada no `.depara_[CNPJ].json`). Sem nenhum nome disponível (exceção), encerre em "...(Orientação Técnica SIT nº 2/2022).", sem a frase final.
...
Esta skill **não toca** em CIF, anexos, fotos ou encoding latin-1. Tudo isso fica com `/aft-gera-ai` quando o AFT empacotar os autos finais.
...
|---|---|
| AFT descreve fato fora da NR-12 | Não force matching. Sinalize: *"Esta irregularidade parece ser de NR-XX, não NR-12. Recomendo /aft-auditoria-geral para identificar a NR correta."* |
