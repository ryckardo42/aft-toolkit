...
**Passo 0a — Revisão pré-empacotamento (`/aft-revisa-auto`).** Antes de injetar o bloco 3,
invoque a skill `/aft-revisa-auto` sobre o `autos.md`. Ela roda o checklist 5W1H, garante o
...
completo em `~/.claude/skills/aft-revisa-auto/SKILL.md`.
...
- Se o auditor não forneceu o código, use a busca em 3 camadas:
  1. **NotebookLM** (se configurado): escolha a key do notebook da NR (`nr-XX` ou
     ...
     em `/aft-notebooklm-login` se ele avisar que não conseguiu reconectar.
...
3. **Onde a pasta é criada** (layout de 22/07/2026): dentro da caixa `AUTOS/` da OS —
   ...
   ainda não migrada; o `/aft-organiza-os` migra o resto depois).
