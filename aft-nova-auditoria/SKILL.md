...
>
> **`## Auditoria de documentos`**: nasce vazia. É o **índice** da análise documental da OS, em duas camadas. No **topo**, as constatações avulsas — o que o AFT apurou sem relatório próprio (SESMT/CIPA subdimensionado, ASO faltando, programa vencido…) — como bullets `- dd/mm/aaaa — texto`, de **até 2 linhas** cada; é o que a `/aft-auditoria-geral` transforma em auto. Abaixo, uma **subseção `### <Tema>` por auditoria temática** (PGR, PCMSO, AET, Jornada, Laudo NR-12…), onde a skill dedicada grava, em **prosa** (nunca bullet), um resumo datado de até 2 linhas terminando no ponteiro do relatório completo, que mora na pasta `auditoria-<tema>/` da OS. O detalhe fica sempre no relatório — a seção nunca repete achados extensos. **Não é lista de tarefas:** não tem caixa de marcar; é o registro do que a auditoria documental encontrou. O painel mostra, permite acrescentar e editar cada constatação.
>
