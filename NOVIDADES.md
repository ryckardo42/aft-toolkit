...
## 29/08/2026
<!-- commit: pasta-preparacao-acao-fiscal -->

**A preparação da ação fiscal ganhou a sua pasta.** Os documentos que abrem uma
fiscalização — o PDF da Demanda, o da Ordem de Serviço, a Relação de Vínculos Ativos
do SFIT e o `preparacao.docx` que se leva impresso na visita — ficavam soltos na raiz
da pasta da empresa. Agora tudo isso mora na subpasta `preparacao-acao-fiscal/`: a
`/aft-preparacao-acao-fiscal` e a `/aft-nova-auditoria` já guardam ali direto, e a
`/aft-organiza-os` inclui no plano, sozinha, a arrumação das pastas antigas que ainda
têm esses arquivos na raiz. A única exceção é o `preparacao.md`, que continua na raiz
da OS — é a ficha que o painel e as outras skills leem por caminho fixo.

---

## 29/08/2026
<!-- commit: ementas-espelho-do-sfit -->

**O arquivo de ementas virou um espelho de verdade da tela do SFIT — e agora aparece no
painel.** Ontem as ementas ganharam arquivo próprio. Faltavam duas coisas, e as duas
saíram hoje.

A primeira: o arquivo separava as ementas em três blocos (as da OS, as que chegaram por
autuação, as que você incluiu). A tela do SFIT não faz isso. Ela tem **uma tabela só**,
agrupada por Atributo/NR em ordem alfabética — DESCANSO, JORNADA, MULH, NR-01, NR-10 — e
o que distingue a ementa da sua Ordem de Serviço é o **asterisco** ao lado do código,
como diz a legenda da própria tela. Uma ementa autuada fora da OS não vai para um bloco
no fim: ela entra no grupo dela, no meio das outras. Espelho que reorganiza a tela não
serve para conferir linha a linha, então o arquivo passou a ter a mesma ordem, com o
mesmo asterisco. (A coluna "Ocorrência" continua de fora de propósito: ela traz
informação do banco do próprio SFIT, como "Fiscalização anterior" ou "Autuação
Obrigatória", que o toolkit não tem — coluna sempre vazia só atrapalha a conferência.)

A segunda, e é a mais útil no dia de encerrar: **o painel agora mostra as ementas.** No
dossiê de cada auditoria, logo abaixo do endereço, apareceu um destaque com quantas
ementas existem, quantas já foram respondidas, quantas faltam e quantas estão
irregulares, e o botão **"abrir o espelho do SFIT"**. Ele abre a tabela inteira em tela
cheia, na mesma ordem e com as mesmas colunas do SFIT-WEB: situação encontrada, ações a
serem aplicadas e comentários. A ação "Autuação" vem marcada como *o SFIT preenche* — é
a que chega sozinha quando o auto é transmitido, e você não digita. Quem está sem
resposta aparece com a borda tracejada, e há filtros para ver só o que falta responder
ou só as irregulares.

A tela tem endereço próprio, então dá para deixá-la **numa janela ao lado do SFIT-WEB**
enquanto você digita — que é exatamente o momento em que ela serve. Também imprime bem,
se preferir no papel. O painel só lê o arquivo: nada ali é alterado por ele.

Você não precisa fazer nada. As auditorias que já tinham o arquivo no formato de ontem
continuam abrindo normalmente, e passam para o formato novo na primeira vez que forem
gravadas.

---

## 29/08/2026
<!-- commit: nco-introducao-notificada -->

**A abertura da notificação para correção volta a dizer sempre "NOTIFICADA", e o título mudou.** Desde 26/08 a notificação trocava sozinha a abertura por "fica a empresa ORIENTADA" quando todos os itens nasciam como orientação. Parecia coerente, mas era uma armadilha: o tipo de cada item é editável no rascunho do DET, e basta o AFT promover um item de orientação a exigência de cumprimento de obrigação para a abertura, já colada, passar a contradizer os próprios itens — sem nada avisando. A natureza da exigência é assunto de cada item, não do texto de abertura.

Agora existe uma abertura só, sempre com o verbo NOTIFICADA, e ela não muda por causa dos itens. O bloco de observações também deixou de ter duas versões: ganhou uma frase — "Item sem prazo e sem retorno tem o cumprimento verificado em fiscalização futura" — que é uma regra por item e por isso continua verdadeira nos dois casos, seja a notificação toda de obrigação, toda de orientação ou misturada. Nenhuma palavra do texto padrão foi removida.

**O título da notificação passou de "Termo de Notificação" para "Notificação para Cumprimento de Obrigações".** Vale para as notificações novas; as já lavradas no DET ficam como estão.
...
## 28/08/2026
<!-- commit: resumo-notificacoes-painel -->

**O painel agora diz O QUE cada notificação DET cobra — e as mostra na ordem em
que foram lavradas.** Até aqui, o card de uma notificação dizia quando ela foi
lavrada e quando vence, mas não do que ela trata: para lembrar, era preciso
abrir o PDF. Agora o Sincronizar busca no próprio DET o título e os itens de
cada notificação lavrada e grava na ficha uma linha de resumo, que o painel
mostra no card com o marcador 📄 — por exemplo: "📄 TN · 7 obrigações:
Mobiliário dos Postos de Trabalho; Avaliação Ergonômica Preliminar; …". Dá para
saber de relance se é uma NAD pedindo documentos ou um Termo de Notificação
exigindo obrigações, e quais são os primeiros itens. O resumo é gravado uma
única vez por notificação (o conteúdo de notificação lavrada não muda), então
só custa uma consulta extra na primeira sincronização depois desta atualização.
De quebra, os cards de notificação passam a aparecer ordenados pela data de
lavratura — a cronologia real da fiscalização — em vez da ordem em que entraram
na ficha. Notificação cancelada e rascunho não ganham resumo.
...
## 28/08/2026
<!-- commit: pastas-tematicas-auditoria -->

**Cada auditoria de documento agora tem a sua pasta, e a "Auditoria de documentos"
virou um índice limpo.** Antes, os relatórios de análise (PGR, PCMSO, AET, jornada,
laudo de NR-12, acidente) se acumulavam soltos na raiz da pasta da empresa, e a seção
"Auditoria de documentos" da ficha ia inchando com parágrafos longos a cada análise —
ficava difícil até de ler. Agora cada tema tem a sua subpasta (`auditoria-PGR`,
`auditoria-jornada`, `auditoria-AET`...), onde a skill guarda tudo o que produz:
relatório, extrato do documento e autos redigidos. Na ficha, a seção passa a ter duas
camadas: em cima, as constatações avulsas de sempre (curtinhas, no máximo duas linhas
cada, que continuam virando auto pela auditoria geral e continuam editáveis pelo
painel); embaixo, uma subseção por tema com um resumo de até duas linhas apontando o
relatório completo. O painel mostra os temas agrupados no cartão, e o link do relatório
abre com um clique.

Três acertos vieram junto: a análise de PGR e a de AET, que não deixavam nenhum
registro na ficha, agora registram o resumo; os dimensionamentos de CIPA e SESMT
passaram a anotar a constatação como o de banheiros (NR-24) já fazia; e uma análise
feita em conversa comum, fora de skill (uma auditoria de convenções coletivas, por
exemplo), agora pergunta se você quer guardar o relatório na pasta do tema e registrar
o resumo — nada mais fica solto na raiz.
...
## 28/08/2026
<!-- commit: tn-nco-relatorio-leem-auditoria -->

**A notificação de correção e o relatório final agora enxergam a Auditoria de
documentos.** Duas lacunas antigas: a `/aft-tn-nco`, que é justamente a habilidade da
irregularidade sem auto, não olhava a seção onde essas irregularidades ficam
registradas; e a `/aft-relatorio` montava o relatório final sem contar o que a análise
documental apurou. Agora a notificação de correção junta às candidatas as constatações
da seção que ainda não viraram auto (as carimbadas ficam de fora, como sempre) e, nas
auditorias temáticas (PGR, jornada...), segue o apontamento e lê o relatório completo
na pasta do tema. O relatório final usa o mesmo material como contexto da narrativa —
descrevendo o que foi examinado, sem jamais apresentar como autuado o que não tem auto
transmitido. Nada muda no que você decide: o checklist da notificação continua sendo
seu, item a item.
...
## 27/08/2026
<!-- commit: aft-toolkit-visual-identity -->

**Os documentos do toolkit agora seguem a identidade visual oficial da SIT.** Em abril
deste ano a Secretaria de Inspeção do Trabalho publicou o seu padrão de comunicação visual
(Manual dos Manuais, modelo de apresentação e Papel Timbrado AFT). As cores do toolkit
tinham sido escolhidas a olho antes disso e não batiam com ele; agora batem. O azul dos
títulos passou a ser o do logotipo AFT do Papel Timbrado, e o azul dos subtítulos e das
tabelas, o do brasão da SIT. A capa ganhou o fio dourado de divisão entre o título e o
subtítulo, e toda página passou a sair com o fio fino cinza-azulado e o "Página N" à
direita, como no papel timbrado oficial. Os títulos e subtítulos passaram a usar a Public
Sans, a fonte oficial da SIT — se ela não estiver instalada no seu Word, ele troca sozinho
por outra parecida, e o documento continua saindo certo. **O corpo do texto não mudou:
continua em Times New Roman 12, justificado, que é a leitura jurídica de sempre**, e o
cabeçalho institucional segue exatamente como estava.

Vale para todo `.docx` que o toolkit gera — relatório de fiscalização, triagem da ação
fiscal, relatório de acidentes, relatório de análise de acidente, documentos avulsos. O
Relatório Técnico de interdição/embargo e a Relação de autos ficaram de fora de propósito:
têm template oficial próprio.

Junto veio um recurso pedido no uso: para **citar um artigo de lei ou um item de NR** dentro
de um documento existe agora um quadro azul próprio. Até aqui só havia a caixa vermelha, que
é de alerta — ela continua reservada ao que é grave (embaraço à fiscalização, fraude), e
citação de norma deixa de sair com cara de acusação.
...
## 26/08/2026
<!-- commit: pdf-texto-paginado-colisao-temporario -->

**Dois documentos de mesmo nome não se atrapalham mais na leitura.** Quando uma análise
tem vários documentos — a resposta da empresa ao DET com um PDF por item, ou PGR, AET e
laudo de NR-12 juntos —, o toolkit lê todos ao mesmo tempo, um leitor para cada. O
rascunho de texto que cada leitor gerava era batizado apenas pelo nome do arquivo, e
resposta de empresa costuma trazer nomes repetidos em pastas diferentes (o mesmo
`01 2025.pdf` nos recibos e nas folhas de pagamento). Dois leitores disputavam o mesmo
rascunho e um apagava o do outro **sem nenhum erro na tela** — o extrato podia sair
descrevendo o documento errado.

Agora cada documento tem rascunho próprio, batizado pelo caminho completo, e a gravação
só é dada por concluída quando termina, para ninguém ler arquivo pela metade. Junto,
corrigimos a instrução do leitor de documentos: ela mandava usar sempre um mesmo nome de
rascunho, o que fazia os leitores colidirem até com documentos de nomes diferentes.

Nada muda no seu uso: os extratos saem iguais aos de antes. O que muda é que o texto de
um documento não tem mais como aparecer no lugar do outro.
...
elas sumiam sem aviso.** Se você tem um modelo no DET com blocos de observação (o
texto que vai depois dos itens), eles nunca chegavam à NAD: bastava a notificação
ter introdução — e ela sempre tem, porque a própria skill manda copiar a
introdução legal literalmente — para o toolkit descartar, calado, todas as
observações do modelo. Não havia erro na tela: a notificação simplesmente saía
sem elas.

Agora o modelo completa **grupo a grupo**: a introdução do modelo entra se o seu
arquivo não trouxe introdução, e as observações do modelo entram se ele não trouxe
observações. O que você escreveu continua mandando, e nunca é substituído.

**O que muda na prática:** se você usa modelo do DET com observações, a próxima
notificação vai sair com elas — texto que antes ficava de fora. A prévia mostra
todos os blocos antes de criar o rascunho, e o assistente diz quais vieram do
modelo em vez do seu arquivo. Confira, como sempre, antes de lavrar; se não quiser
um bloco do modelo, escreva a sua própria seção de observações no arquivo, que ela
prevalece.
...
## 23/08/2026
<!-- commit: cartao-esocial-painel -->

**O cartão do eSocial no painel da OS agora mostra férias e folha.** O cartão que se
chamava "LRE eSocial" virou **"eSocial"** e, além dos ativos e do indício de registro
tardio, ganhou duas linhas de números quando as análises novas já rodaram: férias
(vencidas, gozo fora do prazo, prazo vencendo) e folha (buracos, ano sem 13º, sem base
rescisória) — em vermelho o que merece atenção. E em vez de um link, até três: **abrir o
painel do LRE**, **painel de férias** e **painel da folha**, cada um em outra aba.

Como antes, o cartão só mostra números agregados, sem nome e sem CPF; os painéis
completos continuam abrindo só no modo interativo, servidos localmente de dentro da
pasta da OS. Quem ainda não rodou as análises de férias e folha vê o cartão como sempre
foi, só com o LRE.
...
## 22/08/2026
<!-- commit: metas-regularizacao-preparacao -->

**A preparação da ação fiscal agora destaca as ementas que contam para a meta do
projeto.** Nos projetos de SST, a fiscalização só conta para a meta quando o AFT
consegue a regularização de pelo menos 2 ementas de gradação I3 ou I4 (na construção
civil: 3, e somente das NR-10, NR-18 e NR-35). A `/aft-preparacao-acao-fiscal` ganhou
uma fase nova: logo depois de registrar as ementas da OS, um script cruza cada código
com a base de gradação do ementário SST (5.434 ementas, embutida na skill) e mostra,
em destaque, quais ementas da OS são I3/I4 — e, entre elas, quais são de fácil
regularização pelo empregador (lista curada com nota de 8 a 10 e a justificativa:
relação de máquinas, procedimentos escritos, revisão de PGR, treinamentos de mercado,
comunicação prévia de obra). O quadro entra no chat, numa seção própria do
`preparacao.md` e nos pontos de atenção da visita, com os lembretes de praxe: ementa
alvo de embargo/interdição conta mesmo sem regularização, e empresa sob dupla visita
não é autuada de imediato, mas a regularização conta. A gradação nunca é dita de
cabeça: vem sempre da base local, e código fora dela é apontado para conferência.
A lista de fáceis é tática de meta, não limite: nenhuma ementa da OS deixa de ser
fiscalizada por causa dela — quem decide a estratégia é o AFT.
...
## 21/08/2026
<!-- commit: aft-det-baixar -->

**Nova skill /aft-det-baixar: baixar os arquivos do DET pelo chat.** O mesmo
motor do botão "⬇ baixar arquivos" do painel, agora como skill oficial: peça
"baixa a notificação CÓDIGO do DET" (ou pelo CNPJ/nome da empresa) e os
arquivos chegam em segundos na pasta da OS — o PDF da notificação, o Relatório
de Atendimento e os itens entregues pelo empregador, tudo dentro de
`notificacao-<CÓDIGO>/`, organizados por item. Precisa do mesmo gesto de
sempre: um **Sincronizar** da extensão Sync DET nos últimos 25 minutos; se a
janela passou, a skill pede em uma frase e continua. Nada de navegador
clicando sozinho, nada de senha: o token fica no servidor do painel e não
passa pela conversa.

---

## 21/08/2026
<!-- commit: pacote-notificacoes-com-data -->

**Notificações baixadas: pacote em NOTIFICACOES/, com data no nome.** Duas
arrumações na convenção de pastas, valendo para o botão "⬇ baixar arquivos",
para a skill /aft-det-baixar e para o /aft-organiza-os. Primeira: todo pacote
de notificação mora dentro da pasta `NOTIFICACOES/` da OS — a raiz da
fiscalização fica só com as fichas e documentos de trabalho. Segunda: o nome
do pacote perde o prefixo "notificacao-" e ganha a data do primeiro download:
`SFJHL84A8VWD4A 21-08-2026`. Baixar de novo a mesma notificação (entrega
parcelada, prorrogação aceita) acumula no mesmo pacote, sem duplicar nada.
Quem tem pastas no formato antigo não precisa fazer nada: no próximo download
o pacote é renomeado ao padrão e PDFs soltos são movidos para dentro —
apelidos que você tenha dado à pasta (`XYZ jornada`) são respeitados.
...
## 21/08/2026
<!-- commit: historico-canal-relatorio -->

**O download do DET agora conta a história inteira da notificação.** Três
avanços num pacote. Primeiro: os **pedidos de prorrogação, justificativas e
mudanças de status** de cada item viram um arquivo legível
(`historico-itens.md`) dentro do pacote da notificação — uma notificação "sem
documentos" deixa de parecer vazia: dá para ver que a empresa pediu prazo, e
quando. Segundo: o **canal de comunicação** vem junto — as mensagens trocadas
com o empregador em `canal-comunicacao/mensagens.md`, os anexos delas e o
histórico oficial em PDF; e no painel o aviso ✉️ agora mostra **o que o
empregador escreveu** (o trecho aparece no cartão após o próximo Sincronizar).
Tudo somente leitura: responder e registrar ciência continuam sendo atos do
AFT, no site. Terceiro: o **Relatório de Atendimento é sempre atualizado** a
cada download — antes ele era baixado uma vez e ficava velho quando a empresa
entregava mais documentos depois.

---

## 21/08/2026
<!-- commit: token-automatico-extensao-v3 -->

**Fim do "Sincronizar" manual só para baixar: o token chega sozinho (extensão
v3.0).** Até agora, depois de uns 25 minutos o painel "esquecia" a autenticação
do DET e era preciso clicar Sincronizar de novo antes de cada download — e esse
clique ainda demorava, porque varria todas as auditorias. A extensão Sync DET
v3.0 passa a **enviar o token ao painel sozinha**, enquanto você navega no DET:
o botão "⬇ baixar arquivos" e a skill /aft-det-baixar ficam prontos sem clique,
e a espera some (o envio do token não varre mais nada). O painel também passou
a respeitar o prazo real do token: aproveita os ~30 minutos cheios, em vez de
25 fixos. Uma coisa não muda, e é honesto dizer: o **login no DET a cada ~30
minutos continua** — ele é do sistema do governo (o acesso vence e não há
renovação automática); o que acabou foi a burocracia do nosso lado. Para valer,
a nova versão da extensão precisa ser publicada na Chrome Web Store.
...
**A seção "Anotações da auditoria" virou "Auditoria de documentos", e agora é registro, não
lista de tarefas.** É onde você anota o andamento da análise documental: a irregularidade
que achou no PGR, o ASO que falta, a ata de CIPA sem assinatura, o que quer que tenha
verificado nos papéis depois da inspeção física. Por isso saiu a caixinha de marcar e saiu o
botão "tratada" — uma constatação não é uma tarefa que se resolve, é um fato que fica
registrado. No lugar dele, cada linha ganhou um botão **"editar"**: você clica, o texto vira
um campo ali mesmo, corrige e salva (`Enter` salva, `Esc` desiste). O nome da seção muda
sozinho na ficha da OS quando você registra ou edita alguma coisa pelo painel; as fichas
antigas, com o nome e o formato de antes, continuam sendo lidas normalmente.

**Os botões de dentro das listas agora parecem botões.** O "resolver" das Pendências passou a
se chamar **"resolvido"** e ganhou fundo cor de barro, igual ao "editar" da Auditoria de
documentos. Antes eram brancos sobre cartão branco e mal dava para ver que havia ali algo
clicável.

## 20/08/2026
<!-- commit: painel-edita-inspecao -->

**O relato da inspeção física agora se corrige pelo painel.** Você narra a visita, o Claude
grava o `inspecao-fisica.md` — e meia hora depois lembra de uma máquina que ficou de fora, ou
percebe que trocou o nome de um setor. Até agora não havia o que fazer no painel: era abrir o
arquivo. Agora o cartão **Inspeção física** tem um botão **"editar"** ao lado do título. Clicando,
o relato inteiro vira um campo de texto — o arquivo como ele está, do cabeçalho ao último item —
e você corrige a frase, acrescenta uma lembrança no fim ou registra o que viu numa **nova visita**
à mesma empresa. Como é texto longo, aqui o `Enter` serve para quebrar linha: quem salva é o botão
**salvar** (ou `Ctrl`/`⌘` + `Enter`); `Esc` desiste. Antes de gravar, o toolkit guarda uma cópia do
arquivo original em `.backups/`, então nenhuma edição errada perde o relato de antes. É o mesmo
padrão de botão da Auditoria de documentos e das Pendências.
...
## 20/08/2026
<!-- commit: pendencias-secao-padrao -->

**Agora dá para registrar pendência pelo painel — e toda ficha nasce com a seção.** A
`## Pendências` (aquela lista do que você ainda precisa fazer ou conferir na OS) existia, mas
nunca tinha entrado no modelo da ficha: só aparecia quando alguma skill escrevia nela. Resultado:
em várias auditorias não havia onde anotar "cobrar o AEJ de julho na próxima visita", e o cartão
nem aparecia no painel. Duas coisas mudaram. Primeiro, **`## Notificações DET`, `## Auditoria de
documentos` e `## Pendências` passam a ser obrigatórias em toda ficha**, mesmo vazias — é onde o
painel escreve, e sem a seção o campo não tem onde gravar. Segundo, o cartão **Pendências da OS**
ganhou um campo **"nova pendência"** com botão **"registrar"**, igual ao da Auditoria de
documentos: você escreve, aperta `Enter` ou clica, e a linha entra no `memory.md` na hora. O cartão
agora aparece mesmo quando não há pendência nenhuma, justamente para você poder registrar a
primeira. Quem já tem OS abertas não precisa fazer nada: a seção que faltar é criada sozinha na
primeira vez que você registrar alguma coisa.
...
Árvore de Causas.** A correção de 11/08 tirou o vocabulário errado; esta atualização troca
o esqueleto inteiro da análise, com roteiro proposto por um auditor a partir de uso real
(análise de acidente grave, validada e aprovada). O que muda no seu dia a dia:

- **A análise parte do trabalho habitual, não do acidente.** Primeiro se reconstrói o
  sistema funcionando normalmente (a situação de referência); depois se pergunta o que
  variou no dia do evento. É a comparação que revela o mecanismo do acidente — inclusive
  a diferença entre o que mudou naquele dia e a condição que sempre existiu (o estado
  permanente), que sozinha não explica por que o acidente ocorreu justamente ali.
- **O texto principal sai pronto para o campo "Informações adicionais" do SFITWEB**, em
  quatro seções (trabalho habitual, o que variou, árvore de causas, fatores em três
  níveis), autossuficiente e em texto puro para colar. O que não cabe no campo (jornada e
  fadiga, apreciação da investigação da empresa, lacunas, medidas, encaminhamentos) vai
  para o .docx.
- **Irregularidade não é automaticamente causa.** A skill agora separa expressamente as
  infrações com nexo demonstrado com o acidente das que devem ser autuadas mas não
  integram a malha causal.
- **Fadiga é investigada de verdade**, com teste objetivo no ponto/AFD (descanso anterior,
  horas de jornada no momento do evento, prorrogação no dia, depoimentos) — sem suporte,
  o fator não é adotado, e a razão fica registrada.
- **A investigação da própria empresa vira objeto de apreciação crítica:** contradição
  entre a árvore e a conclusão, uso do paradigma vencido, e plano de ação carregado de
  medidas comportamentais no lugar de medidas na fonte.
- **A consulta às fontes de regência (Guia de 2010, Caminhos de 2003 e IN nº 2/2022) é
  obrigatória e prévia**, logo na Fase 0.

Tabela de fatores SFIT, script do .docx e o encadeamento para `/aft-auditoria-geral` e
