...
description: >
  ...
  clandestino", "empregado não registrado". Gera dois autos completos (art. 41 + art. 29)
  no formato consumido pelo /gera-ai, que faz o empacotamento no TXT do Sistema Auditor.
---

# registro — Autos de Infração: Falta de Registro (art. 41) e CTPS (art. 29)
**AFT Toolkit**
...
Esta skill **redige** os dois autos; o empacotamento do TXT é do `/gera-ai`.
...
2. **`[atividade_economica]`** — atividade econômica do estabelecimento
3. **Lista de trabalhadores irregulares** — para cada um: nome completo, função e data/tempo de admissão (converter para "admitido(a) X dias antes da inspeção")
4. **`[data_eSocial]`** — data(s) em que foi consultado o eSocial
5. **ME/EPP** — a empresa é Microempresa ou Empresa de Pequeno Porte?
6. **Pasta da OS** — qual empresa em `~/Documents/AFT/OS ATIVAS/` (para salvar o texto e o de-para)
...
Reproduza estes textos literalmente em ambos os autos, nos subtítulos indicados:

**[BLOCO_FISC]:**
> *Trata-se de fiscalização mista, realizada nos termos do art. 30, § 3º, do Decreto nº 4.552/2002, iniciada em **[data_inspecao]** e ainda em curso na presente data no empregador acima qualificado, que desenvolve a atividade econômica de **[atividade_economica]**.*
...
Monte os dois blocos no formato consumido pelo `/gera-ai`. **Texto puro**, sem markdown.
...
(1- [[TRAB_01]] - Função, admitida(o) X dias antes da inspeção.)
...
Em consulta ao banco de dados do eSocial, realizada no(s) dia(s) [data_eSocial],
...
- **Lista idêntica** nos dois autos. Numeração sequencial: `1- [[TRAB_01]] - Função, admitida(o) X dias antes da inspeção.`
...
2. **Salve os dois blocos** em `~/Documents/AFT/OS ATIVAS/[PASTA_EMPRESA]/autos-registro.md` (sobrescreve com aviso).
3. Apresente os dois autos para revisão do AFT (com tokens).
4. Handoff:

```
✅ Autos art. 41 + art. 29 redigidos — [[AUTUADA]]
...
▶ Próximo passo — gerar o TXT do Sistema Auditor:
  ...
  3) Os trabalhadores citados entram como linhas tipo 4 (o /gera-ai usa o de-para já criado).
```
