...
gerado pela `/aft-autos-lavrados`) e os PDFs de notificação DET ainda não registrados na
...
Com `--scan`, o gerador chama o `scan_autos.py` (da `/aft-autos-lavrados`) para cada OS com
...
O artefato é um retrato: só muda quando o `/aft-painel` rodar e publicar de novo.
