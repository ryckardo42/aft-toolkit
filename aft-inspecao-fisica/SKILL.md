...
da fiscalização — depois a `/aft-PGR-analise` o lê para confrontar a realidade in loco contra o
PGR, e a `/aft-auditoria-geral` o usa como base factual para redigir os autos.
...
da `/aft-auditoria-geral` e da `/aft-PGR-analise`. Aqui, só os fatos.
...
(`[[TRAB_NN]]`), e o TXT final do Sistema Auditor é re-hidratado por script (ver `/aft-gera-ai`).
...
- **Apareceu um CPF/PIS:** avise o AFT em uma linha — *"Detectei 1 CPF no relato; confirme que
  ele entra no de-para antes do `/aft-gera-ai`."* O nome do trabalhador em contexto permanece no
...
> "Salvei em `inspecao-fisica.md`. Quer que eu siga para `/aft-PGR-analise` (confronta estes
> achados de campo contra o PGR da empresa) ou `/aft-auditoria-geral` (redige os autos de
