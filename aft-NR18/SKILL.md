...
- **Serra circular é NR-18** (`3182908`, item 18.10.1.5), mas qualquer outra máquina/equipamento genérico é **NR-12** — direcione para `/aft-NR12`.
...
Para cada ementa confirmada, redija o **bloco 2)** que será colado dentro do auto de 3 subtítulos. A skill chamadora (`/aft-auditoria-geral` ou o próprio AFT) cuida dos subtítulos 1 e 3.
...
Esta skill **não toca** em CIF, anexos, fotos, dados da autuada ou encoding latin-1. Tudo isso fica com `/aft-gera-ai` quando o AFT empacotar os autos finais.
...
|---|---|
| AFT descreve fato de outra NR (altura, EPI, máquina genérica) | Não force matching. Sinalize a NR correta e a skill (ex.: `/aft-NR12` para máquina, NR-35 para altura). |
...
| Serra circular | É NR-18 (`3182908`). Outras máquinas → `/aft-NR12`. |
